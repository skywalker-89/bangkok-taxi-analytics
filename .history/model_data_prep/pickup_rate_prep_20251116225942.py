...
def setup_database_indexes():
    ...
    try:
        ...
        if "idx_timestamp" not in existing:
            print("  Creating timestamp index (5-10 min)...")
            cur.execute(
                """
                CREATE INDEX CONCURRENTLY idx_timestamp 
...
def process_window(args):
    """Process a single window - optimized for speed"""
    ...
    # OPTIMIZATION: Use indexed columns and only select what we need
    query = f"""
    ...
    try:
        ...
        print(f"Error querying {train_start.date()}: {e}")
        engine.dispose()
...
def create_pickup_features():
    """Main function with parallel processing and optimizations"""
    try:
        # Setup indexes first
        ...
        print(f"\n⚠️  WARNING: Processing {len(window_starts)} windows.")
        print(
            f"   Estimated time: {len(window_starts) * 2 // 60} - {len(window_starts) * 5 // 60} minutes"
        )
        ...
        print(f"🔄 [2/4] Processing windows in parallel...")
        ...
        with Pool(N_WORKERS) as pool:
            results = []
            for i, result in enumerate(pool.imap(process_window, args_list)):
