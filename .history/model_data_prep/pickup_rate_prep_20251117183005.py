...
H3_RESOLUTION = 8  # Changed from 9 to 8 for 2-3x speedup
TIME_BIN_MINUTES = 5
...
def process_window(args):
    """
    Process a single window - now queries DB per window but uses indexes
    """
    ...
    # Query with indexed timestamp - should be fast
    query = f"""
...
def create_pickup_features():
    """Main function - memory-efficient approach with database queries"""
    try:
        # Setup indexes first - CRITICAL for performance
        ...
        print(
            f"\n🔄 [2/3] Processing {len(window_starts)} windows with {N_WORKERS} workers..."
        )
        print(f"   Strategy: Query DB per window (memory-efficient)")
        print(f"   Estimated time: 30-90 minutes with indexes\n")
        ...
        print(f"\n✅ [3/3] Processed {len(results)} windows successfully")
