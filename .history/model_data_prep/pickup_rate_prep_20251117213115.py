...
def add_time_features(df):
    ...
    df["dow"] = df["time_bin"].dt.dayofweek.astype("int8")
    df["hour"] = df["time_bin"].dt.hour.astype("int8")
...
def process_window_streaming(window_start, window_config, engine):
    ...
    # Query database - use LIMIT to prevent memory explosion
    query = f"""
    ...
    # CRITICAL: Limit number of cells to prevent memory explosion
    all_cells = pickups_df["h3_cell"].unique()
    if len(all_cells) > MAX_CELLS_PER_WINDOW:
        # Keep only the most active cells
    ...
    # Create base grid - optimize memory
    ...
    y = X_with_target[["y"]]
    meta = X_with_target[["h3_cell", "time_bin"]]
    X = X_with_target.drop(columns=["y", "h3_cell", "time_bin"])
...
def create_pickup_features():
    ...
    NO multiprocessing to avoid memory issues on M2 MacBook Air
    """
    try:
        # Setup indexes first
        ...
        print(f"   Strategy: One window at a time (memory-safe for M2 Air)")
        print(f"   Estimated time: 45-120 minutes\n")
        ...
        # Initialize output files (write headers)
        first_write = True
        windows_processed = 0
