...
def load_data():
    ...
    meta["time_window_start"] = pd.to_datetime(meta["time_window_start"])
...
def split_data(X, y, meta):
    ...
    split_time = meta["time_window_start"].quantile(0.85)

    train_indices = meta[meta["time_window_start"] < split_time].index
    valid_indices = meta[meta["time_window_start"] >= split_time].index
...
def train():
    ...
    # The X_train and X_valid dataframes now contain the encoded column
    ...
    with mlflow.start_run() as run:
        ...
        mlflow.log_param("data_split_time", meta_train["time_window_start"].max())
        ...
        # Best params from notebook (you can tune these)
        params = {
