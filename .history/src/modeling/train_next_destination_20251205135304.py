...
ENCODER_FEATURE_FILE = "dest_feature_encoder.pkl"
ENCODER_TARGET_FILE = "dest_target_encoder.pkl"

# --- CONFIGURATION ---
ARTIFACT_DIR = "artifacts"
os.makedirs(ARTIFACT_DIR, exist_ok=True)
ENCODER_FEATURE_FILE = os.path.join(ARTIFACT_DIR, "dest_feature_encoder.pkl")
ENCODER_TARGET_FILE = os.path.join(ARTIFACT_DIR, "dest_target_encoder.pkl")
TOP_K_DESTINATIONS = 100  # ⚡ Limit to top 100 zones for speed
...
def train():
    ...
    # --- 1. Filter Top K Destinations (Performance Fix) ---
    print(f"✂️ Filtering to Top {TOP_K_DESTINATIONS} destinations...")
    target_counts = df["h3_end"].value_counts()

    # Take only the top K most frequent zones
    valid_targets = target_counts.head(TOP_K_DESTINATIONS).index

    initial_len = len(df)
    df = df[df["h3_end"].isin(valid_targets)].copy()
    print(f"   Reduced dataset from {initial_len} -> {len(df)} rows.")
    print(f"   (Removed tail zones to speed up training by ~40x)")
    ...
    print(f"🚀 Training Multi-Class Classifier on {num_classes} classes...")
    ...
    with mlflow.start_run():
        ...
        mlflow.log_param("top_k_classes", TOP_K_DESTINATIONS)
        ...
        # Safe Top-K calculation
        k_eval = min(5, num_classes)
        all_labels = range(num_classes)
        top3 = top_k_accuracy_score(
            y_test, y_proba, k=min(3, num_classes), labels=all_labels
        )
        top5 = top_k_accuracy_score(y_test, y_proba, k=k_eval, labels=all_labels)
