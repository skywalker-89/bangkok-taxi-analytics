...
class BangkokTaxiOptimizer:
    ...
    def __init__(self):
        ...
        try:
            # Load ALL models to make realistic predictions
            # Using 'None' fetches the latest version
        ...
        # ✅ FIX: Get Run ID directly from the Registered Model (More Robust)
        print("⏳ Downloading feature encoders...")
        try:
            # Get details of the latest version of the destination model
            # This is safer than searching experiments
            ...
            latest_version = sorted(
                latest_versions, key=lambda x: x.version, reverse=True
            )[0]
            run_id = latest_version.run_id
            print(f"   Using Artifacts from Run ID: {run_id}")
            ...
            # Don't crash app entirely, just warn (allows debugging other parts)
            print("⚠️ Warning: running without encoders. Predictions will fail.")
    ...
    def predict_trip_metrics(self, start_h3, end_h3, current_time):
        ...
        """
        # Convert H3 to Lat/Lon for distance calc
        ...
        # 2. Predict Duration
        try:
            # We try to use the model, if columns mismatch we fallback
    ...
    def predict_next_step(self, current_h3, current_time):
        # Safety check if encoders failed to load
