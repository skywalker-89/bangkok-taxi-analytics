...
def api_optimize_route() -> Any:
    try:
        ...
        def serialize_route(route):
            ...
            # ✅ FIX: Ensure revenue is pulled correctly
...
def api_route_geojson(route_index: int) -> Any:
    try:
        ...
        # --- 2. Process Each Trip (Lines AND Markers) ---
        for i, trip in enumerate(trips):
            ...
            # Marker: Pickup Point (Shows Trip Number)
            features.append(
                {
                    "type": "Feature",
                    "geometry": {
            ...
            # Connection from previous dropoff to current pickup (Empty Leg)
            if i > 0:
