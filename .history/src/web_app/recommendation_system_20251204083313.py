...
# ✅ FIX: Increase timeouts for MLflow operations
...
class BangkokTaxiOptimizer:
    ...
    def __init__(self):
        ...
        try:
            ...
            try:
                ...
                print("   Artifact download failed/skipped (using local if present).")

            if os.path.exists("dest_feature_encoder.pkl"):
                self.le_start = joblib.load("dest_feature_encoder.pkl")
                self.le_end = joblib.load("dest_target_encoder.pkl")
                print("✅ Encoders loaded.")
            else:
                print("⚠️ Warning: Encoder files missing.")
                self.le_start = None

        except Exception as e:
            print(f"❌ Error logic in encoder loading: {e}")
    ...
    def predict_trip_metrics(self, start_h3, end_h3, current_time):
        ...
        try:
            if hasattr(self, "le_start") and self.le_start:
                s_idx = (
    ...
    def predict_next_step(self, current_h3, current_time):
        if not hasattr(self, "le_start") or not self.le_start:
            return []
        try:
    ...
    def optimize_route(
        ...
        for i in range(n_simulations):
            ...
            for _ in range(3):
                ...
                # ✅ FIX: Use Real Models for Price
                if self.distance_model and self.duration_model:
                    pred_dist, pred_dur = self.predict_trip_metrics(
                        current_h3, dest_h3, current_time
                    )
                else:
                    pred_dist, pred_dur = 5.0, 20.0  # Fallback
