...
def haversine(lat1, lon1, lat2, lon2):
    ...
    # Convert to numpy arrays, ensuring 1D
    lat1_rad = np.radians(np.asarray(lat1).ravel())
    lon1_rad = np.radians(np.asarray(lon1).ravel())
    lat2_rad = np.radians(np.asarray(lat2).ravel())
    lon2_rad = np.radians(np.asarray(lon2).ravel())
...
def prepare_inter_zone_data():
    ...
    try:
        ...
        # Add ORDER BY to the query for better performance
        query = """
        ...
        for i, df in enumerate(df_chunks):
            ...
            # Already sorted by query, no need to sort again
        ...
        # Build new dataframe with correct column names to avoid duplicates
        empty_trips = pd.DataFrame(
            {
                "vehicle_id": empty_trips["vehicle_id"].values,
                "trip_id": empty_trips["trip_id"].values,
                "start_time": empty_trips["prev_dropoff_time"].values,
                "start_lat": empty_trips["prev_end_lat"].values,
                "start_lon": empty_trips["prev_end_lon"].values,
                "end_time": empty_trips["pickup_time"].values,
                "end_lat": empty_trips["start_lat"].values,
                "end_lon": empty_trips["start_lon"].values,
            }
        )
        ...
        print(f"Filtered to {len(empty_trips)} valid empty trips.")

        # --- 4. Time, Geo, and H3 Features ---
        print("🛠️ [4/7] Engineering time, geo, and H3 features...")

        # Time features (vectorized - fast)
        ...
        # Bearing features (vectorized - fast)
        bear = bearing(
            empty_trips["start_lat"],
            empty_trips["start_lon"],
            empty_trips["end_lat"],
        ...
        # H3 features (list comprehension is faster than apply)
        print("  Converting to H3 hexagons (this may take a minute)...")
        empty_trips["start_h3"] = latlng_to_h3_vectorized(
            empty_trips["start_lat"].values,
            empty_trips["start_lon"].values,
            H3_RESOLUTION,
        )
        empty_trips["end_h3"] = latlng_to_h3_vectorized(
            empty_trips["end_lat"].values, empty_trips["end_lon"].values, H3_RESOLUTION
        )

        # Distance from center (vectorized - fast)
        ...
        # a. Historical OD Pair Counts (fast with groupby)
        ...
        print("✂️ [7/7] Separating features (X) and target (y)...")
