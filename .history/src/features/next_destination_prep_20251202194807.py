...
def train():
    ...
    # --- 1. Encode Features (h3_start) ---
    print("🔑 Encoding Start Locations (Features)...")
    # We fit this on ALL zones to be safe for inference inputs
    ...
    # --- 2. Encode Targets (h3_end) - THE FIX ---
    print("🔑 Encoding Destinations (Targets)...")
    # XGBoost Classifier requires targets to be 0, 1, 2... N with NO GAPS.
    # So we MUST fit a dedicated encoder strictly on the existing targets.
    le_target = LabelEncoder()
    df["h3_end_idx"] = le_target.fit_transform(df["h3_end"])

    # Save Target Encoder (Need this to decode predictions back to H3 strings)
    ...
    # Remove rare classes (appearing < 2 times) to prevent train_test_split errors
    ...
    print(f"✂️ Splitting data (removed {len(df) - len(X)} rare samples)...")
    ...
    with mlflow.start_run():
        ...
        # Calculate Top-K
        # Handle edge case: if test set has fewer classes than trained model,
        # we must pass the full list of labels to score correctly.
